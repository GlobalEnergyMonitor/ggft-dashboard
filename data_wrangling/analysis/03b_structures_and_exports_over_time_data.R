...
gcpt_status_over_time_df <-
  ...
  writexl::write_xlsx(gcpt_status_over_time_df, 
                      file.path(project_data_dir,
                                year_filename,
                                "download_files",
                                paste0("gcpt_", year_filename, "_status_over_time.xlsx")))

# turns the dataframe into a json file, using the toJSON function from the jsonlite package
gcpt_status_over_time_json <- 
  jsonlite::toJSON(gcpt_status_over_time_df |>
                     mutate(across(everything(), as.character)),
                   simplifyDataFrame = TRUE,
                   pretty = TRUE)

# Saves out the final data as a json file, to be uploaded to the dashboard folder
writeLines(gcpt_status_over_time_json, 
           file.path(project_data_dir,
                     year_filename,
                     "output",
                     paste0("gcpt_", year_filename, "_status_over_time.json")))
